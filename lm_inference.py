...
import models  # pylint: disable=unused-import # type: ignore
import t5.data # type: ignore
from transformer import inference_utils # type: ignore
...
class LanguageModelInference:
  ...
  def call(
      self,
      inputs: np.ndarray, # type: ignore
      dstate: tuple[dict[str, np.ndarray], ...] = None, # type: ignore
      eos: np.ndarray = None, # type: ignore
  ...
  def beam_decode(
      self,
      inputs: str,
      eos_tokens: np.ndarray = None, # type: ignore
      mask_tokens: np.ndarray = None, # type: ignore
      dstate: dict[str, np.ndarray] = None, # type: ignore
  ) -> MetricsOutput:
    """Beam search."""
